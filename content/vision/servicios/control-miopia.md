---
# 2.1.3 — Control de Miopía
title: "Control de miopía y Orto-K en Torre del Mar | Ópticas Fausto"
linkTitle: "Control de Miopía"
description: "Frena el avance de la miopía en niños y jóvenes con ortoqueratología (lentes nocturnas) y lentillas de desenfoque periférico. Estudio con topógrafo corneal en Torre del Mar."
page_id: "2.1.3"
weight: 30

card:
  description: "Frena el avance de la miopía en niños y jóvenes."
  image: "/images/cards/213-miopia.webp"
  imageAlt: "Estudio de control de miopía con topógrafo corneal en Ópticas Fausto"

og:
  title: "Frenar la miopía es posible — Control de miopía y Orto-K"
  description: "Lentes nocturnas que corrigen la visión mientras duermes y frenan el avance de la miopía. Estudio personalizado con topógrafo corneal."
  image: "/images/og/213-miopia.jpg"
  type: "article"

schema:
  type: Service
  serviceType: "Control de miopía / Ortoqueratología"
  includeBreadcrumb: true

hero:
  title: "La miopía de tu hijo no tiene por qué seguir subiendo"
  subtitle: "Cada revisión, media dioptría más. Si esa frase te suena, no estás solo: la miopía se ha convertido en un problema creciente en Europa, especialmente entre niños y adolescentes. La buena noticia es que hoy **es posible frenar su avance** — y una de las técnicas más eficaces trabaja mientras tu hijo duerme."
  preset: contact
  image: "/images/213-optometria_D3A0075.webp"
  imageAlt: "Un óptico-optometrista realiza una retinografía a una niña, con los títulos y certificados colgados en la pared"

sections:
  - type: faq
    title: "Preguntas frecuentes sobre el control de miopía"
    items:
      - question: "¿A qué edad se puede empezar?"
        answer: "No hay una edad mínima estricta: depende de la madurez del niño y de su caso concreto. Lo valoramos en el estudio inicial. Cuanto antes se actúe sobre una miopía en progresión, mayor es el beneficio acumulado."
      - question: "¿Es seguro dormir con las lentes Orto-K?"
        answer: "Sí, siempre que la adaptación la realice un profesional cualificado con topógrafo corneal y se sigan las pautas de higiene y revisión. El efecto es además reversible: si se deja de usar la lente, la córnea recupera su forma."
      - question: "¿Cuándo se nota el efecto?"
        answer: "La mayoría de los pacientes nota una mejora clara en los primeros días, alcanzando la visión óptima en un plazo corto que varía según la graduación de partida."
      - question: "¿Cómo sabemos que está funcionando?"
        answer: "Con revisiones periódicas medimos tanto la calidad de visión diurna como la evolución de la miopía a lo largo del tiempo. No es una sensación: son datos comparados cita a cita."
      - question: "¿Y si mi hijo ya lleva gafas?"
        answer: "No es ningún problema: el estudio parte de su graduación y su historia actual. De hecho, si su miopía ha subido en las últimas revisiones, es el candidato típico para valorar un tratamiento de control."

  - type: cta
    preset: contact
    title: "El avance de una miopía no se nota en una visita. Se ve comparando muchas."
    subtitle: "El control de miopía es, por definición, un tratamiento de seguimiento: lo que importa es la curva a lo largo de los años, no la foto de un día. Nosotros seguiremos aquí en cada revisión — las mismas personas, midiendo con el mismo criterio, comparando con los mismos datos."
    microcopy: "Estudio con topógrafo corneal · Valoración sin compromiso"
    labelCall: "Llamar ahora"
---

## Orto-K: ver bien todo el día, sin gafas ni lentillas

La ortoqueratología (Orto-K) utiliza unas lentes especiales **que se llevan solo durante la noche**. Mientras duermes, moldean suavemente la curvatura de la córnea, de forma que al despertar ves con nitidez durante todo el día — sin gafas y sin lentes de contacto.

Es una técnica reversible y muy segura, pero **requiere conocimientos especializados y un topógrafo corneal**, el instrumento que permite mapear la córnea y adaptar la lente de forma correcta e individualizada a cada paciente. En Ópticas Fausto contamos con el equipo y la formación para hacerlo.

{{< image-text image="/images/211-optometria_D3A7499.webp" imageAlt="Topógrafo corneal en Ópticas Fausto, mapa de curvatura de la córnea en pantalla" >}}
El mapa de curvatura de tu córnea, en pantalla, antes de adaptar cualquier lente — no se improvisa.
{{< /image-text >}}

### ¿Para quién está recomendada?

- **Niños y adolescentes** con miopía en progresión — el grupo donde el control del avance es más importante. Suele detectarse en la [revisión visual infantil](/vision/servicios/vision-infantil/).
- **Opositores** y profesiones que exigen buena visión sin corrección (cuerpos de seguridad, etc.).
- **Deportistas** y personas activas que no quieren depender de gafas ni lentillas durante el día.
- Quienes, por sus condiciones oculares, **no pueden o no quieren optar por la cirugía refractiva**.

{{< banner bg="/images/213-miopia_ortok_sueno.webp" textSize="xl" pad="l" >}}Corrige tu miopía mientras duermes.{{< /banner >}}

## Lentillas blandas de desenfoque periférico

Además de la Orto-K, la investigación actual en control de miopía se centra en **lentes de contacto blandas de diseño especial** (como las lentillas MiSight), que han demostrado ralentizar significativamente la progresión de la miopía en niños y jóvenes. Puedes conocerlas en nuestra página de [lentes de contacto](/vision/productos/lentes-de-contacto/).

Ambos tratamientos se basan en el mismo mecanismo: el modo en que las imágenes se enfocan en la retina central y periférica, que parece ser el principal responsable del crecimiento del ojo — y por tanto, del aumento de la miopía.

En el estudio inicial valoramos qué opción encaja mejor con la edad, el estilo de vida y las características oculares de cada paciente.

{{< banner bg="/images/213-miopia_hero.webp" pad="l" >}}## Hay momentos que quieres recordar con nitidez.{{< /banner >}}

{{< cta title="Pide tu estudio de control de miopía" microcopy="Estudio con topógrafo corneal · Valoración sin compromiso" >}}
