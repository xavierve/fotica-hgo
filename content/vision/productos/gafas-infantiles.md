---
# 2.2.6 — Gafas Infantiles (landing SEM Fase 2 · vuelta al cole)
title: "Gafas infantiles resistentes en Torre del Mar | Ópticas Fausto"
linkTitle: "Gafas Infantiles"
description: "Gafas chulas e irrompibles para niños, con revisión visual completa gratuita. Monturas resistentes, lentes de policarbonato y filtros de pantalla en Torre del Mar."
page_id: "2.2.6"
weight: 60

card:
  description: "Chulas e irrompibles, para que las usen y duren."
  image: "/images/cards/226-infantil.webp"
  imageAlt: "Gafas infantiles resistentes en Ópticas Fausto"

og:
  title: "Gafas chulas e irrompibles — con revisión visual gratuita"
  description: "Monturas infantiles resistentes, adaptadas a su edad y su cara, con revisión visual completa gratis. Torre del Mar, desde 1982."
  image: "/images/og/226-infantil.jpg"
  type: "article"

schema:
  type: WebPage
  includeBreadcrumb: true

hero:
  title: "Gafas chulas e irrompibles"
  subtitle: "**Para que las usen — y para que duren.** Unas gafas infantiles tienen dos exámenes que aprobar: que a tu hijo le gusten lo suficiente como para no querer quitárselas, y que sobrevivan al patio, al deporte y a la mochila. En Ópticas Fausto encontrarás monturas y gafas de sol infantiles de todos los materiales y tamaños, adaptadas a su defecto visual, su edad y su forma facial — sin limitar ni su actividad diaria ni su deporte."
  preset: contact
  image: "/images/226-infantil_hero.webp"
  imageAlt: "Dos niñas con gafas de colores, espalda contra espalda, sobre fondo claro"

sections:
  - type: faq
    title: "Preguntas frecuentes sobre gafas infantiles"
    items:
      - question: "¿La revisión visual es realmente gratuita?"
        answer: "Sí, la revisión visual completa infantil es gratuita y sin compromiso. Preferimos que salgas de dudas a que lo dejes pasar."
      - question: "¿Y si rompe las gafas?"
        answer: "Trabajamos con monturas pensadas para resistir — y cuando aun así pasa (son niños), la mayoría de los ajustes y pequeñas reparaciones los resolvemos en el día, en cualquiera de los dos centros."
      - question: "¿Cómo consigo que las lleve puestas?"
        answer: "Participando él o ella en la elección: cuando la montura les gusta, la batalla está medio ganada. Nuestro equipo tiene mucha experiencia (y mucha paciencia) ayudando a los niños a elegir."
      - question: "¿Cada cuánto hay que cambiarlas?"
        answer: "La graduación se revisa cada año (los ojos de los niños cambian rápido) y la montura cuando se queda pequeña o deja de ajustar bien. En cada revisión valoramos ambas cosas."
      - question: "¿Hacen deporte con las gafas puestas?"
        answer: "Para deporte habitual, mejor una montura resistente bien ajustada; para deportes de contacto o intensos, existen [gafas deportivas homologadas](/vision/productos/gafas-deportivas/) o [lentillas para niños y jóvenes](/vision/productos/lentes-de-contacto/)."

  - type: cta
    preset: contact
    title: "Sus gafas crecen con él. Y nosotros vamos ajustándolas por el camino."
    subtitle: "Los niños crecen, aprietan, doblan y estiran. Por eso las gafas infantiles necesitan más visitas que las de un adulto — ajustes, apretar tornillos, revisar que siguen centradas. Aquí esas visitas son bienvenidas, rápidas y las atiende quien ya conoce a tu hijo por su nombre. Pásate cuando quieras: para eso estamos a la vuelta de la esquina."
    microcopy: "Revisión visual completa gratis · Ajustes y pequeñas reparaciones en el día"
    labelCall: "Llamar ahora"
---

{{< banner >}}Pide cita para una revisión visual completa — **¡GRATIS!**{{< /banner >}}

## El 80% de lo que aprende entra por los ojos

El 80% de la información que llega a nuestro cerebro entra a través de la vista. Una dificultad visual no corregida puede convertirse en un problema de aprendizaje — y el niño no avisará, porque [no sabe que ve mal](/vision/servicios/vision-infantil/): para él, el mundo siempre se ha visto así.

### Señales de que necesita una revisión

- Visión borrosa o doble
- Se acerca en exceso al cuaderno o la pantalla
- Guiña o cierra un ojo con frecuencia
- Gira la cabeza o el cuerpo al leer o escribir
- Sigue la línea del texto con el dedo
- Invierte o confunde letras al leer o escribir
- Dolores de cabeza

¿Reconoces alguna? La revisión completa es gratuita y sale de dudas en media hora.

{{< image-text image="/images/212-vision_infantil_revision.webp" imageAlt="Óptico de Ópticas Fausto realizando una revisión visual infantil" reverse="true" >}}
Una revisión de verdad, no un vistazo — con pruebas pensadas para la edad del niño, sin necesidad de que sepa leer.
{{< /image-text >}}

## Antes de los libros, la revisión

Cada septiembre nos encontramos lo mismo: niños que llevaban meses viendo mal sin que nadie lo supiera — con dolores de cabeza, fatiga o notas que no reflejan su esfuerzo. Una revisión antes del curso localiza el problema a tiempo y evita descubrirlo a mitad de trimestre, cuando ya ha pasado factura.

Y ya que estamos: la audición también cuenta en el rendimiento escolar. Si tu hijo pide que le repitan las cosas o sube mucho el volumen, [revisemos también su oído](/audicion/).

{{< cta title="Reserva su revisión antes del curso" microcopy="Revisión visual completa gratuita · Sin compromiso" >}}

## Resistentes al ritmo de un niño, elegidas con criterio

- **Monturas prácticamente indestructibles** — materiales flexibles y memoria de forma, pensados para caídas, tirones y mochilas.
- **Lentes de policarbonato** — el material más resistente al impacto y muy ligero: el estándar de seguridad infantil. → [Más sobre materiales de lentes](/vision/productos/lentes-oftalmicas/)
- **Protección frente a pantallas** — filtro de luz azul disponible para las horas de tablet, ordenador y consola. → [Filtros de luz azul](/vision/productos/filtros-solares-luz-azul/)
- **Gafas de sol infantiles** — los ojos de los niños son más sensibles al UV que los de los adultos; la protección solar certificada no es un capricho.
- **Adaptadas a cada cara y cada edad** — el ajuste correcto no es estética: unas gafas que bailan no corrigen bien.

{{< image-text image="/images/226-infantil_resistencia_card3.webp" imageAlt="Niña demostrando la flexibilidad de una montura infantil de Ópticas Fausto" >}}
No es solo un mensaje comercial — la montura está pensada para doblarse antes que romperse.
{{< /image-text >}}
