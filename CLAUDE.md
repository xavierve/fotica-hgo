...
8. **Hero: pasar de `background-image` a `<picture>`/`<img>` real, con layout apilado en móvil.**
   ...
   **Criterio de aceptación:** build limpio con y sin variantes `_hd`/recortes móvil creados; ninguna imagen rota si falta un archivo opcional; el layout apilado en móvil se ve y se comporta como bloque en flujo normal (no absolute/fixed); el texto se lee bien en ambos breakpoints, sin heredar blanco donde ya no hay foto detrás; el H1 y el subtítulo son visibles sin scroll en un móvil estándar (ver aviso de altura arriba); verificar visualmente en varios anchos de viewport, no solo que compile.
